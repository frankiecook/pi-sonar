...
def echo():
    for i in range(360/ROTATION_ANGLE):
        ...
        points.append(Point(distance, ROTATION_ANGLE * rotation_counter))
        Spin()
        rotation_counter += 1
        plotPoint(points[i])
...
grid.pack()
startButton = Button(rFrame, command = start_button, text = "Start", bg = "white", height = "13", width = "27")
startButton.pack(fill = BOTH)
calButton = Button(rFrame, command = calibrate_button, text = "Calibrate", bg = "white", height = "13", width = "27")
calButton.pack(fill = BOTH)
stopButton = Button(rFrame, command = stop_button, text = "Stop", bg = "white", height = "12", width = "27")
