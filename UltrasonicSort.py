...
# Name: James R. Henry
# Date: 07/12/19
# Description: A program that utilizes a simple circuit with an ultrasonic sensor to detect
# distances from the sensor and then store, sort and display them
###########################################################################################
...
def sortMeasurements(distances):
    n = len(distances)
    for i in range(0, n - 1):
        minDistance = i
        for j in range(i + 1, n):
            if (distances[j] < distances[minDistance]):
                minDistance = j
    temp = distances[i]
    distances[i] = distances[minDistance]
    distances[minDistance] = temp
    
########
# MAIN #
########

# first, allow the sensor to settle for a bit_length
...
# create a list for the measured distances to be stored
distances = []
...
while (True):
    ...
    # store the measured distance in a list
...
# finally, cleanup the GPIO pins, and print the stored measurements
print ("Done.")
print ("Unsorted measurements: {}").format(distances)
sortMeasurements(distances)
print ("Sorted measurements: {}").format(distances)
